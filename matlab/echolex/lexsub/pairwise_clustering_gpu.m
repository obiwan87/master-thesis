...
X = gpuArray(m.X(w2v_u_i,:));
dist_u = double(squareform(gather(pdist(X,'cosine'))));
%dist_u = gknnsearch(m.X(w2v_u_i,:), m.X(w2v_u_i,:), numel(w2v_u_i), 'cosine');
dist_u(dist_u > 1) = 1;
