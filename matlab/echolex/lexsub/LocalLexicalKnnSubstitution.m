classdef LocalLexicalKnnSubstitution < LexicalSubstitutionPreprocessor
    ...
    methods               
        ...
        function r = doExecute(obj, ~, args)
            ...
            results = LocalLexicalKnnSubstitution.do(D.V,F,D.I,D.m.X,D.Vi,...
                obj.K,obj.DictDeltaThresh,obj.MaxIter); 
            
            LD = io.Word2VecDocumentSet(D.m, results(end).DocumentSet, D.Y);
            r = struct('Out', LD);
            info = struct();
    ...
    methods(Static)
        function results = do(V, F, I, X, Vi, K, DictDeltaThresh, MaxIter, IterationCallback)           
            if nargin < 9
