...
% u = {'Rt10k'}; %unique(best_results.DatasetName(:));
% 
% for j=1:numel(u)
...
f = strcmp(best_results.DatasetName, 'Rt10k');
best_results = best_results(f,:);
...
for j=1:size(best_results, 1)
    ...
    accNb = NaN; % trainTestNB(mergedDataset, mergedDataset.W, train_ind, test_ind);
    subAccNb = NaN; %trainTestNB(mergedSubstitutedDataset, mergedSubstitutedDataset.W, train_ind, test_ind);
