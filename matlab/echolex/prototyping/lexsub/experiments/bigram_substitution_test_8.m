...
for lk=1:numel(Ws)
    ...
    for N=minN:maxN
        ...
        all_results = cell(numel(trainingSetSizes),1);
        ...
        for ll = 1:runs
            ...
            for lj=1:numel(trainingSetSizes)
                ...
                % Store results here
                accuracies2_n_nb =  nan(abs_fold, size(param_combinations,1));
                precisions2_n_nb = zeros(abs_fold, size(param_combinations,1));
                recalls2_n_nb = zeros(abs_fold, size(param_combinations,1));
                
                accuracies2_n_svm =  nan(abs_fold, size(param_combinations,1));
                ...
                for i = 1:abs(fold)
                    ...
                    for li = 1:size(param_combinations,1)
                        ...
                        % Calculate best parameter combination sofar
                        bests_nb = mean(accuracies2_n_nb, 'omitnan');
                        bests_svm = mean(accuracies2_n_svm, 'omitnan');
                        
                        [~, best_nb] = sort(bests_nb(~isnan(bests_nb)), 'descend');
                        [~, best_svm] = sort(bests_svm(~isnan(bests_svm)), 'descend');
                        
                        bests_idx = [best_nb; best_svm];
                        bests = [bests_nb; bests_svm];
                        bests_names = {'NB', 'SVM'};
                        ...
                        for kk=1:numel(bests_names)
                            for nn=1:min(3, numel(best_nb))
                                ...
                                cutoff = param_combinations{best_idx,1}(N);
                                
                                a = param_combinations{best_idx,2}(N);
                                b = param_combinations{best_idx,3}(N);
                                
                                maxDistance = param_combinations{best_idx,4}(N);
                                name = bests_names{kk};
                                
                                fprintf('%d. best so far for %s: \n cutoff: %.2f, a: %.2f, b: %.2f, maxDistance: %.2f\n Accuracy: %.2f\n', ...
                                    nn, name, cutoff, a, b, maxDistance, best);
                                disp('--------------------------------------------');
                            end
                            disp('============================================');
                        end
