...
param_fields = 1:6;
best_params_folder = fullfile(echolex_dumps, 'lincomb_10_all');
prefix = 'lincomb_10 - s';
...
for lk=1:numel(Ws)
    ...
    for N=minN:maxN
        ...
        %% load best parameters from ref folder        
%         results_filename = sprintf('%s-%d-grams.mat',EW.DatasetName,NN-1);
%         load(fullfile(best_params_folder, results_filename));
%         [TID, ~, groups] = unique(all_accs_t(:,2:5));
%          
%         acc = splitapply(@mean, all_accs_t.mean_acc_sub_n_svm , groups);
%         acc = [acc splitapply(@mean, all_accs_t.mean_acc_sub_n_nb , groups)]; %#ok<AGROW>
...
%         best_combination = table2cell(TID(best_idx,:));
        ...
        %params_ngrams = cell2mat(best_combination);
        ...
        %% Generate N-Grams
        ngramsEW = BigramFinder.generateAllNGrams(EW,NN,true);
            ...
            for lj=1:numel(folds)
                ...
                for i = 1:abs(fold)
                    ...
                    for li = 1:size(param_combinations,1)
                        ...
                        sTrD = trD_uni.applySubstitution(training_substitution_map);
                        sTeD = teD_uni.applySubstitution(substitution_map);
                        ...
                        sD = BigramFinder.generateAllNGrams(sD, NN, true);
        ...
        results_filename = sprintf('%s-%d-s-grams.mat',EW.DatasetName,NN);
