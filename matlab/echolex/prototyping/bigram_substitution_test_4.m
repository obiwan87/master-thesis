...
for ll = 1:runs
    ...
    for lj=1:numel(folds)
           ...
           % [acc1_bi, p] = trainTestNB(ngramsEW, bigramsWC, trainingIdx, testIdx);
            ...
            for li = 1:size(param_combinations,1)
                ...
                for jj=1:N
                    ...
                    cutoff = param_combinations{li,jj}(1);
                    a = param_combinations{li,jj}(2);
                    b = 1 - a;
                    
                    maxDistance = param_combinations{li,jj}(3);
                    fprintf('%d-grams: cutoff: %.2f, a: %.2f, maxDistance: %.2f, linkage: %s \n \n',jj,cutoff, a, maxDistance, linkag);
        ...
        for li=1:size(param_combinations)
            cutoff = zeros(1,N);
            a = zeros(1,N);
            maxDistance = zeros(1,N);
            for jj = 1:N
                cutoff(jj) = param_combinations{li,jj}(1);
                a(jj) = param_combinations{li,jj}(2);
                maxDistance(jj) = param_combinations{li,jj}(3);            
            end
