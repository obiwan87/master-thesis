...
crossvalParams = allcomb({'holdout'}, num2cell(0.5:0.1:0.9));
crossvalParams = arrayfun(@(x) {crossvalParams{x,1}, crossvalParams{x,2}}, 1:size(crossvalParams,1), 'UniformOutput', false);

p = sequence(fork(pgrid('ExcludeWords', 'MinCount', 1:3, 'MaxCount', Inf)), ...
             fork(pgrid('NGrams', 'NBest', 10:20:110, 'ScoreFcn', {'raw_freq'})), ...
             WordCountMatrix(), ...
