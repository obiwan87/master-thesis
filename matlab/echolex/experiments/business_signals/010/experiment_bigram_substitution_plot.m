...
query ='[{$match:{ExperimentId:10}},{$unwind:"$Steps"},{$project:{dataset:"$Dataset",classifier:{$arrayElemAt:["$Steps.Name",-1]},method:{$arrayElemAt:["$Steps.Name",0]},accuracy:"$Steps.Out.accuracy",holdout:{$arrayElemAt:["$Steps.Args.crossvalparams",0]},bigrams:{$arrayElemAt:["$Steps.Args.nbest",0]}}},{$project:{dataset:1,classifier:1,method:1,accuracy:1,holdout:{$arrayElemAt:["$holdout",1]},bigrams:1,_id:0}},{$unwind:"$accuracy"},{$sort:{bigrams:1,holdout:1}},{$group:{_id:{holdout:"$holdout",method:"$method",classifier:"$classifier",dataset:"$dataset"},accuracy:{$push:"$accuracy"},bigrams:{$push:"$bigrams"}}},{$sort:{"_id.classifier":1,"_id.dataset":1,"_id.holdout":1,"_id.method":1}},{$match:{"_id.dataset":"%s","_id.classifier":"%s"}}]';
